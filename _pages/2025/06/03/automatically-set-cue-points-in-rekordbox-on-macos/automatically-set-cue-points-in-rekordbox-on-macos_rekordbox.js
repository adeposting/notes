...
To use this script, you need to first set up the keyboard shortcuts in 
Rekordbox. In the menu bar, go to rekordbox => Preferences => Keyboard.
Import the rekordbox.mappings file, it is in the same directory as this 
script. You can also set the keyboard shortcuts this manually as follows: 
Select "Performance 1 (Preset) in the drop-down menu. Expand the section for
"Deck 1". Ensure the following key shortcuts are set. 
...
(() => {
	...
	// - Set pad mode to Hot Cue
	function setPadModeToHotCue() {
		console.log("Set pad mode to Hot Cue")
		systemEvents.keystroke("[");
	}
	
	// - Move to Hot Cue A
	function moveToHotCueA() {
		console.log("Move to Hot Cue A")
		systemEvents.keystroke("]");
		systemEvents.keystroke("[", { using: "shift down" });
		delay(SHORT_DELAY);
		systemEvents.keystroke("s");
		delay(SHORT_DELAY);
	}
	...
	function setAllMemoryCuesByNumberOfBars(stepBarCount) {
	...
	function runSetMemoryCuesFromFirstBeatOfTrackScript(numberOfTracks, trackGenre) {
		...
		iterateThroughTracksWithCallback(numberOfTracks, function () {
			...
			deleteAllMemoryCues();
			setAllMemoryCuesByNumberOfBars(moveByNumberOfBars);
		});
	}
	
	function runSetMemoryCuesFromHotCueAScript(numberOfTracks, trackGenre) {
		moveByNumberOfBars = getNumberOfBarsToMoveForwardByForMemoryCuesByGenre(trackGenre);
		iterateThroughTracksWithCallback(numberOfTracks, function () {
			moveToFirstBeatOfTrack();
			deleteAllMemoryCues();
	...
	function runSet1Point1BarsAtFirstBeatOfTrackScript(numberOfTracks) {
		iterateThroughTracksWithCallback(numberOfTracks, function () {
			moveToFirstBeatOfTrack();
			set1Point1Bars();
		});
	...
	function runSet1Point1BarsAtHotCueAScript(numberOfTracks) {
		iterateThroughTracksWithCallback(numberOfTracks, function () {
			moveToHotCueA();
			set1Point1Bars();
		});
	...
	function getScriptFunction(scriptAction, trackGenre, numberOfTracks) {
		switch (scriptAction) {
			case "Set memory cues from first beat of track":
				return () => runSetMemoryCuesFromFirstBeatOfTrackScript(numberOfTracks, trackGenre);
			case "Set memory cues from hot cue A":
					return () => runSetMemoryCuesFromHotCueAScript(numberOfTracks, trackGenre);
			...
			case "Set 1.1 bars at first beat of track":
				return runSet1Point1BarsAtFirstBeatOfTrackScript(numberOfTracks);
			case "Set 1.1 bars at hot cue A":
				return runSet1Point1BarsAtHotCueAScript(numberOfTracks);
			default:
				// do nothing
				break;
		}
